...
from src.cli import run_batch_evaluate
...
@patch("src.cli.answer.Evaluator")
@patch("src.cli.answer.Storage")
@patch("src.cli.answer.ThreadPoolExecutor")
def test_run_batch_evaluate_with_csv(
    mock_executor, mock_storage, mock_evaluator, tmp_path
):
    ...
    mock_executor.return_value.__enter__.return_value.submit.side_effect = (
        lambda fn, *args: (fn(*args), future)[1]
    )
    ...
    from src.cli import run_batch_answer

    run_batch_answer(input_dir=out_dir, input_csv=csv_file, api_key="dummy")
